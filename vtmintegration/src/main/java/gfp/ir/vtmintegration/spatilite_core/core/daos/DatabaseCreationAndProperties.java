...
import gfp.ir.vtmintegration.spatilite_core.core.enums.SpatialiteVersion;
 ...
 */
public class DatabaseCreationAndProperties implements gfp.ir.vtmintegration.spatilite_core.core.daos.ISpatialiteTableAndFieldsNames {
     ...
     * @return the {@ link gfp.spatilite_core.core.enums.SpatialiteVersion}.
     * @throws Exception if something goes wrong.
     */
    ...
    public static String getSpatialiteProperties(Database database) throws Exception {
        String s_value = "-";
        Stmt stmt = database
                .prepare("SELECT HasIconv(),HasMathSql(),HasGeoCallbacks(),HasProj(),HasGeos(),HasGeosAdvanced(),HasGeosTrunk(),0,HasLibXML2(),HasEpsg(),HasFreeXL();"); //FIXME: HasRtTopo(), is removed from this
        try {
            if (stmt.step()) {
