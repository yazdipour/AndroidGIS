...
public class GeoDatabaseHandler {
    ...
    public String CopyDbfile(String dbname){

        String DB_PATH;
            if (android.os.Build.VERSION.SDK_INT >= 17)
                DB_PATH = context_.getApplicationInfo().dataDir + "/databases/";
            else
                DB_PATH = "/data/data/" + context_.getPackageName() + "/databases/";


        File cacheDatabase = new File(DB_PATH, dbname);
            ...
            return cacheDatabase.getAbsolutePath();
    }
    Context context_;
    public GeoDatabaseHandler(Context context) throws IOException {
        ...
        if (android.os.Build.VERSION.SDK_INT >= 17)
         ...
         cacheDatabase = new File(DB_PATH, DB_NAME);
        if (!cacheDatabase.getParentFile().exists()) {
            File dirDb = cacheDatabase.getParentFile();
            Log.i(TAG,"making directory: " + cacheDatabase.getParentFile());
            if (!dirDb.mkdir()) {
