...
import editormodule.layerprovider.GeoserverTMSProvider;
...
import static editormodule.utils.AppSettingsConstants.KEY_PREF_SHOW_COMPASS;
import static editormodule.utils.AppSettingsConstants.KEY_PREF_SHOW_MEASURING;
import static editormodule.utils.AppSettingsConstants.KEY_PREF_SHOW_SCALE_RULER;
import static editormodule.utils.AppSettingsConstants.KEY_PREF_SHOW_ZOOM;
import static editormodule.utils.AppSettingsConstants.KEY_PREF_SHOW_ZOOM_CONTROLS;

/**
 * Main map com.editor.fragment
 */
public class MapFragment
    ...
    private void addlayers() {
        new LoadLayers().execute(mMap, getActivity().getApplication());
    }
    ...
    public boolean saveEdits() {
        ...
        if (mSelectedLayer != null) {
            if (featureId == NOT_FOUND) {
                //show attributes edit com.editor.activity
...
//        if (mEditLayerOverlay.hasEdits()) TODO prompt com.editor.dialog
//            return;
...
//        toolbar.getBackground().setAlpha(128);
        ...
        switch (mode) {
            ...
            case MODE_SELECT_ACTION:
                ...
                mFinishListener = new OnClickListener() {
                    @Override
                ...
                toolbar.setOnMenuItemClickListener(
                        new BottomToolbar.OnMenuItemClickListener() {
                            ...
                            public boolean onMenuItemClick(MenuItem item) {
                                ...
                                if (item.getItemId() == R.id.menu_feature_add) {
                                    ...
                                    mUndoRedoOverlay.saveToHistory(mEditLayerOverlay.getSelectedFeature());
                                    mEditLayerOverlay.setHasEdits(true);
                                } else if (item.getItemId() == R.id.menu_feature_edit) {
                                    setMode(MODE_EDIT);
                                    mUndoRedoOverlay.saveToHistory(mEditLayerOverlay.getSelectedFeature());
                                    mEditLayerOverlay.setHasEdits(false);

                                } else if (item.getItemId() == R.id.menu_feature_delete) {
                                    deleteFeature();

                                } else if (item.getItemId() == R.id.menu_feature_attributes) {
                                    setMode(MODE_INFO);
            ...
            case MODE_INFO:
                ...
                //get or create com.editor.fragment
                ...
                mFinishListener = new OnClickListener() {
                    @Override
                    public void onClick(View view) {
                        ((EditActivity) getActivity()).finishFragment();
    ...
    public void onLayerDrawFinished(
        ...
        /*if (percent >= 1.0)
            mLayerDrawn++;
        EditActivity com.editor.activity = (EditActivity) mActivity;
        if (null != com.editor.activity){
            if (percent >= 1.0) {
                if (id == mMap.getTopVisibleLayerId()) {
                    com.editor.activity.onRefresh(false, 0);
                } else {
                    com.editor.activity.onRefresh(true, (mLayerDrawn * 100) / mMap.getVisibleLayerCount());
                }
            }
        }*/
    ...
    protected void addNewGeometry() {
        ...
        //show select layer com.editor.dialog if several layers, else start default or custom form
        ...
        if (layers.isEmpty()) {
            ...
            if (isDialogShown())
                return;
            //open choose edit layer com.editor.dialog
    ...
    public void onLongPress(MotionEvent event) {
        ...
        //show actions com.editor.dialog
...
//        return totalWidth < mStatusPanel.getWidth();
    ...
    public void locateCurrentPosition(GeoPoint point) {
        if (mCurrentCenter != null) {
            mMap.panTo(point);
        }
    }
    ...
    public void onClick(View v) {
        ...
        if (v.getId() == R.id.fl_compass) {
            showFullCompass();
        } else if (v.getId() == R.id.add_current_location) {
            if (v.isEnabled())
                addCurrentLocation();
        } else if (v.getId() == R.id.add_new_geometry) {
            if (v.isEnabled())
                addCurrentLocation();
        } else if (v.getId() == R.id.add_geometry_by_walk) {
            if (v.isEnabled())
                addGeometryByWalk();
        } else if (v.getId() == R.id.action_zoom_in) {
            if (v.isEnabled())
                mMap.zoomIn();
        } else if (v.getId() == R.id.action_zoom_out) {
            if (v.isEnabled())
                mMap.zoomOut();
        } else if (v.getId() == R.id.add_point_by_tap) {
            if (mRulerOverlay.isMeasuring()) {
                ...
                mAddPointButton.setImageResource(R.drawable.ic_action_add_point);
                mActivity.setTitle(mActivity.getAppName());
                mActivity.setSubtitle(null);
            } else
                addPointByTap();
        } else if (v.getId() == R.id.action_ruler) {
            startMeasuring();
...
//        }
    ...
    public void onLengthChanged(double length) {
        ...
        //TODO:ADDing a better method to show lenght. such as Toast and so on
...
class LoadLayers extends AsyncTask<Object, Void, Void> {
    protected Void doInBackground(Object... maps) {
        try {
            MapViewOverlays mMap = (MapViewOverlays) maps[0];
            Context context = (Context) maps[1];
//            String LAYER_MBTILESATTLITE = "GoogleSattlite";
//            String LAYER_ONLINE = "onlinelayer";
//            String LAYER_ONLINE_VEC = "geoservervectorlayer";
            File storageDirectory = Environment.getExternalStorageDirectory();
//            if (mMap.getMap().getLayerByName(LAYER_MBTILESATTLITE) == null) {
//                String path = storageDirectory + "/gfp/GoogleTerrain.mbtiles";
//                MbtileTMSProvider mbTile = new MbtileTMSProvider(context,
//                        mMap.getMap().createLayerStorage(LAYER_MBTILESATTLITE), path);
//                mbTile.setName(LAYER_MBTILESATTLITE);
//                mbTile.setVisible(true);
//                mMap.getMap().addLayer(mbTile);
//                mMap.getMap().moveLayer(0, mbTile);
//            }
//            if (mMap.getMap().getLayerByName(LAYER_ONLINE) == null) {
//                String layerURL = "http://207.182.149.126:8080/geoserver/gwc/service/tms/1.0.0/topp:states@EPSG:900913@png/{z}/{x}/{y}.png";
//                final GeoserverTMSProvider layer = new GeoserverTMSProvider(context, mMap.getMap().createLayerStorage(LAYER_ONLINE));
//                layer.setName(LAYER_ONLINE);
//                layer.setURL(layerURL);
//                layer.setTMSType(TMSTYPE_OSM);
//                layer.setVisible(true);
//                layer.setMinZoom(GeoConstants.DEFAULT_MIN_ZOOM);
//                layer.setMaxZoom(19);
//                mMap.getMap().addLayer(layer);
//                mMap.getMap().moveLayer(3, layer);
//            }
//            if (mMap.getMap().getLayerByName(LAYER_ONLINE_VEC) == null) {
//                GeoserverVectorLayer vectorLayer = new GeoserverVectorLayer(context, mMap.getMap().createLayerStorage(LAYER_ONLINE_VEC));
//                vectorLayer.setName(LAYER_ONLINE_VEC);
//                vectorLayer.setVisible(true);
//                vectorLayer.setMinZoom(GeoConstants.DEFAULT_MIN_ZOOM);
//                vectorLayer.setMaxZoom(GeoConstants.DEFAULT_MAX_ZOOM);
//                try {
//                    String url = "http://207.182.149.126:8080/geoserver/topp/ows?service=WFS&version=1.0.0&request=GetFeature&typeName=topp:states&maxFeatures=50&outputFormat=application%2Fjson";
//                    Uri uri = Uri.parse(url);
//                    vectorLayer.createFromGeoJson(uri, null);
//                } catch (Exception e) {
//                    e.printStackTrace();
//                }
//                mMap.getMap().addLayer(vectorLayer);
//                mMap.getMap().moveLayer(4, vectorLayer);
//            }
        } catch (Exception e) {
//            return null;
        } finally {
