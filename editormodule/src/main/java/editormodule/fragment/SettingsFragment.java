...
import editormodule.utils.AppConstants;
import editormodule.utils.IntEditTextPreference;
import editormodule.utils.SelectMapPathPreference;
...
import static editormodule.utils.AppSettingsConstants.KEY_PREF_GA;
    ...
    protected static void deleteLayers(Activity activity) {
        ...
        for (int i = app.getMap().getLayerCount() - 1; i >= 0; i--) {
            ...
            if (!layer.getPath().getName().equals(MainApplication.LAYER_MBTILESATTLITE)
                    && !layer.getPath().getName().equals(MainApplication.LAYERS[0])
                    && !layer.getPath().getName().equals(MainApplication.LAYERS[1])
                    && !layer.getPath().getName().equals(MainApplication.LAYERS[2])) {
                layer.delete();
            }
        }
